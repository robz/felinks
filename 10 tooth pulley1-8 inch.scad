...
motor_shaft = 3.175;	// NEMA17 motor shaft exact diameter = 5
m3_dia = 3.4544;		// 3mm hole diameter
m3_nut_hex = 1;		// 1 for hex, 0 for square nut
m3_nut_flats = 0;	// normal M3 hex nut exact width = 5.5
m3_nut_depth = 0;	// normal M3 hex nut exact depth = 2.4, nyloc = 4

retainer = 1;		// Belt retainer above teeth, 0 = No, 1 = Yes
retainer_ht = 2.15;	// height of retainer flange over pulley, standard = 1.5
idler = 1;			// Belt retainer below teeth, 0 = No, 1 = Yes
idler_ht = 2.15;		// height of idler flange over pulley, standard = 1.5

pulley_t_ht = 4.064;	// length of toothed part of pulley, standard = 12
	 ...
	 {	 
		union()
		{
			...
			if ( pulley_b_ht < 2 ) { echo ("CAN'T DRAW PULLEY BASE, HEIGHT LESS THAN 2!!!"); } else {
				rotate_extrude($fn=pulley_b_dia*10)
				{
		...
		difference()
			...
			cylinder(r=pulley_OD/2,h=pulley_t_ht, $fn=teeth*15);
		...
		translate([0,0,-1])cylinder(r=motor_shaft/2,h=pulley_b_ht + pulley_t_ht + retainer_ht + 2,$fn=motor_shaft*15);
